...
public class StoreServer {
    static String dbfile = "Activity7.db";
        ...
        try {
            ...
            while (true) {
                ...
                if (msg.code == MessageModel.GET_PURCHASE) {
                    ...
                    PurchaseModel pc = adapter.loadPurchase(Integer.parseInt(msg.data));
                    if (pc == null) {
                        msg.code = MessageModel.OPERATION_FAILED;
                    }
                    else {
                        msg.code = MessageModel.OPERATION_OK; // load successfully!!!
                        msg.data = gson.toJson(pc);
                    }
                    out.println(gson.toJson(msg));
                }

                if (msg.code == MessageModel.PUT_PURCHASE) {
                    PurchaseModel pc = gson.fromJson(msg.data, PurchaseModel.class);
                    System.out.println("PUT command with Purchase = " + pc);
                    int res = adapter.savePurchase(pc);
